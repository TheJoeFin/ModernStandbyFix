...
{
    private readonly static string AppPath = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) ?? string.Empty;
    private const string LogName = "Log.txt";
    private static List<string> SuspendedAdapters = new();
    ...
    private static void RegisterForPowerNotifications()
    {
        try
        {
            IntPtr registrationHandle = new();
            DEVICE_NOTIFY_SUBSCRIBE_PARAMETERS recipient = new();
    ...
    /// <param name="type">The type of the callback...for power notification it's a PBT_ message</param>
